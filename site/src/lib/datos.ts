...
export interface ParticipacionPartido {
	partido: string;
...
/**
 * Cruza participación en votaciones (Senado + Diputados + Consejo
 * Regional/gobernador) con el partido declarado, para comparar el
 * promedio de participación por partido. Solo incluye partidos con al
 * menos 2 personas con datos de votación: con 1 sola persona el
 * "promedio" es solo su propio dato individual, no una comparación real.
 */
export function participacionPromedioPorPartido(): ParticipacionPartido[] {
	...
	const indices = [
		...indiceVotaciones(votaciones, senadorIds),
		...indiceVotaciones(votacionesCamara, diputadoIds),
		...indiceVotaciones(votacionesCore, coreIds),
	];

	const porPartido = new Map<string, number[]>();
	for (const i of indices) {
		const autoridad = autoridades.find((a) => a.id === i.autoridad_id);
		if (!autoridad?.partido) continue;
		const partido = PARTIDO_ALIAS[autoridad.partido] ?? autoridad.partido;
		if (!porPartido.has(partido)) porPartido.set(partido, []);
		porPartido.get(partido)!.push(i.participacionPct);
	}
	...
	return [...porPartido.entries()]
		...
		.map(([partido, valores]) => ({
			partido,
			participacionPromedio: Math.round((valores.reduce((s, v) => s + v, 0) / valores.length) * 10) / 10,
			personas: valores.length,
		}))
		.sort((a, b) => b.participacionPromedio - a.participacionPromedio);
}
